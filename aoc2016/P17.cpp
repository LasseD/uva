...
typedef pair<PI,string> PIIS;

int main() {
  GS(s);

  vector<PIIS> v;
  v.push_back(PIIS(PI(0,0),""));
  while(!v.empty()) {
    // Walk all:
    vector<PIIS> v2;
    FORIT(vector<PIIS>, v) {
      ...
      PI p = piis.first;
      int x = p.XX, y = p.YY;
      string path = piis.second;
      if(x == 3 && y == 3) {
	cout << "path" << " of length " << path.size() << endl;
	continue; // Don't continue this path!
      }
      
      string hash = md5(s + path);
      // Up:
      if(y > 0 && isOpen(hash[0])) {
	v2.push_back(PIIS(PI(x,y-1),path+"U"));
      }
      // Down:
      if(y < 3 && isOpen(hash[1])) {
	v2.push_back(PIIS(PI(x,y+1),path+"D"));
      }
      // Left:
      if(x > 0 && isOpen(hash[2])) {
	v2.push_back(PIIS(PI(x-1,y),path+"L"));
      }
      // Right:
      if(x < 3 && isOpen(hash[3])) {
	v2.push_back(PIIS(PI(x+1,y),path+"R"));
      }
    } // FORIT v
    swap(v, v2);
  } // while true
